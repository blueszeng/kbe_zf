...
def onInterfaceAppReady():
	...
	# g_poller.start("192.168.0.13", 30040)
...
def onRequestAccountLogin(loginName, password, datas):
	...
	@type  datas: bytes
	"""
	INFO_MSG('onRequestAccountLogin: registerName=%s %s' % (loginName, datas))
	...
	realAccountName = ''
	dataStr = str(datas, encoding = "utf-8")

	if dataStr == 'wx_login_code':
		''' 微信登录 '''
		args = {
			'openid' : None
		}
		...
		def whenGotToken(content):
			infos = json.loads(content)
			args['openid'] = infos['openid']
			data = bytes(json.dumps((dataStr, infos['access_token'])), encoding='utf-8')
			KBEngine.accountLoginResponse(code, args['openid'], data, KBEngine.SERVER_SUCCESS)


		def whenReject():
			''' 失败时 '''
			KBEngine.accountLoginResponse(code, '', datas, KBEngine.SERVER_ERR_ILLEGAL_LOGIN)
		...
		datas = {
			...
			'secret': 'c2737d3d011bc152088990ae190cd019',
			'code': code,
			}
		doHttpReq(url, datas, whenGotToken, whenReject)
	
	if dataStr == 'phone':
		''' 手机号登录 '''
		def whenGotAccount(response):
			try:
				...
				INFO_MSG(content)
				data = bytes(json.dumps((dataStr,loginName)), encoding='utf-8')
				KBEngine.accountLoginResponse(loginName, str(msg['playerid']), data, KBEngine.SERVER_SUCCESS)
			except Exception as identifier:
				logw(identifier)
				KBEngine.accountLoginResponse(loginName, loginName, datas, KBEngine.SERVER_ERR_ILLEGAL_LOGIN)
		requrl = URL.WEB_is_regist
		request = tornado.httpclient.HTTPRequest(requrl, method='POST', body=json.dumps({
			'account': loginName,
			'password': password,
		}))
		tornado.httpclient.AsyncHTTPClient().fetch(request, whenGotAccount)
	else:
		data = bytes(json.dumps([dataStr]), encoding='utf-8')
		KBEngine.accountLoginResponse(loginName, loginName, data, KBEngine.SERVER_ERR_LOCAL_PROCESSING)


def doHttpReq(url, datas, resolve, reject=None):
	''' http请求 '''
