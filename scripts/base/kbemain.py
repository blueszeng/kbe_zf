...
def onBaseAppReady(isBootstrap):
	...
	# 创建大厅
	if isBootstrap:
		logi('_onCreateHall createBaseFromDBID')
		KBEngine.createBaseFromDBID( "Hall", 1, _onCreateHall )

	KBEngine.addWatcher('userOnline', 'UINT32', countUserOnline)
	KBEngine.addWatcher('rooms', 'UINT32', countRooms)
	KBEngine.addWatcher('GamingRoom', 'UINT32', countGamingRoom)

def _onCreateHall(hall, dbid, wasAlive):
	if not hall:
		logi('_onCreateHall createBaseLocally')
...
def users():
	for e in entities():
		if isinstance(e, KBEngine.Proxy):
			yield e
